...
export async function persistParsedWorkbook(importRecord, parsed) {
    ...
    for (const sheet of parsed.sheets) {
      ...
      await db.datasets.create({
        data: {
          ...
          valid_row_count: sheet.rowCount,
...
export async function getImportDetail(importId, { includeRows = false } = {}) {
  const item = await db.workbook_imports.findUnique({
    where: { id: importId },
    include: {
      datasets: {
...
export async function createMapping(importId, mappingResult, status) {
  const [latest, sourceColumns] = await Promise.all([
    db.schema_mappings.findFirst({
      where: { workbook_import_id: importId },
      orderBy: { version: "desc" },
      select: { version: true },
    }),
    db.dataset_columns.findMany({
      where: { datasets: { workbook_import_id: importId } },
...
export async function saveUserMapping(importId, payload) {
  ...
  const mapping = await createMapping(importId, normalized, "USER_CONFIRMED");
...
export async function getAnalysisSource(importId, mappingId) {
  const item = await db.workbook_imports.findUnique({
    where: { id: importId },
    include: {
      datasets: {
