...
export async function runWorkbookAnalysis(importId, organizationId) {
  ...
  if (cached) {
    return importDetailDto(await getImportDetail(importId, { organizationId }));
  }
  ...
  try {
    const source = await getAnalysisSource(
      importId,
      mapping.id,
      organizationId,
    );
